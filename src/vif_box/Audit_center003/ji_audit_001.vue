<template>
	...
	<div>
	   <el-card v-loading="loading" class="box-card"  style="padding: 0;margin:20px;padding-left:60px;">
		 ...
		 <div v-show="p_val_2"  class="tab_box"><p>机构名称：</p><a>{{p_val_2}}</a></div>
		 <div v-show="p_val_3"  class="tab_box"><p>审核类型：</p><a>{{p_val_3}}</a></div>
		 <div v-show="p_val_4"  class="tab_box"><p>公司名称：</p><a>{{p_val_4}}</a></div>
		 <div v-show="p_val_5"  class="tab_box"><p>经营地址：</p><a>{{p_val_5}}</a></div>
		 <div v-show="p_val_6"  class="tab_box"><p>法人名字：</p><a>{{p_val_6}}</a></div>
		 <div v-show="p_val_7"  class="tab_box"><p>联系人：</p><a>{{p_val_7}}</a></div>
		 <div v-show="p_val_8"  class="tab_box"><p>联系人号码：</p><a>{{p_val_8}}</a></div>
		 <div v-show="p_val_9"  class="tab_box"><p>社会信用代码：</p><a>{{p_val_9}}</a></div>
		 <div v-show="p_val_10"  class="tab_box"><p>经营范围：</p><a>{{p_val_10}}</a></div>
		   ...
		   this.$axios({method:'get',url:store.state.url_data+'/api/auditInstitutions/'+store.state.audit_id,headers:{'Authorization':'Bearer '+localStorage.token}}
		       ...
		       if(res.data.code==200){
		  		  let res_data = res.data.data;console.log(res_data)
				                   ...
				                   if(img_box.business_license){this.url_box[0] = img_box.business_license};
				                   if(img_box.idcard_back){this.url_box[1] = img_box.idcard_back};
								   ...
								   this.p_val_13 = (res_data.log.audit_action==0?'审核不通过':'审核通过');
