<template>
	...
	<div style="width:100%;">
	  ...
	  <div class="box_tall">
		  ...
		  <el-select v-else class='tab_c' @change='lx_cl' v-model="lx_val" placeholder="请选择">
	  ...
	  <div class="box_tall">
		     <p v-if="top_show" style="width:350px;">{{dw_val}}</p>  
	  		  <el-select v-else class='tab_c' @change='dw_cl' v-model="dw_val" placeholder="请选择">
	  		      <el-option v-for='(i,index) in dw_box'  :label="i.name" :value="i.institution_id"></el-option>
	  		  </el-select>
	  		 <p>*所属单位:</p>
	  </div>
	  
	  <div class="box_tall"><el-input class='tab_c' placeholder="请输入内容" v-model="input[0]" clearable></el-input><p>*机构/学校课程名称:</p></div>
      <div class="box_tall"><el-input class='tab_c' placeholder="请输入内容" v-model="input[1]" clearable></el-input><p>*平台课程名称:</p></div>
       
	  <div class="box_tall">
	  		  <el-select class='tab_c' @change='ml_cl' v-model="ml_val" placeholder="请选择">
...
<!-- 封面图  action="https://jsonplaceholder.typicode.com/posts/" -->	
	...
	<p style="float:left;margin-left:100px;width:70%;margin-top:20px;">*课程简介:</p>   
	   <div id="editor2"  style="border:1px solid #ccc;float:left;width:80%;margin-left:110px;"></div>
	   <div id="editor3" class="text" style="float:left;margin-left:110px;"></div>
		...
		git_active(){//提交按钮被点击
		   ...
		   var postData = {
		               ...
		               name:this.input[0],//机构/学校课程名称
		   			   title:this.input[1],//标题
		               revised_name:this.input[1],//运营中心取的名字
		   ...
		   this.$axios({method:type,url:store.state.url_data+'/api/courses/'+sessionStorage.course_id,params:postData,headers:{'Authorization':'Bearer '+localStorage.token}}
		      ).then(res=>{
			   ...
			   this.$axios({method:'get',url:store.state.url_data+'/api/courses/'+sessionStorage.course_id,
			      headers:{'Authorization':'Bearer '+localStorage.token}}
			        ...
			        if(res.data.code==200){
						...
						this.input[1] = datas.revised_name;
	  ...
	  git_file(files){//上传文件函数
	   ...
	   excls(excelObj){//解析excel文件，转成表格html
	     ...
	     let boxs = `<table class="course-table" style="width:100%;float:left;">
		      <tr><th style='padding:10px 20px;border:1px solid #ccc;text-align:center;width: 200px;'>单元</th>
...
//标签选择函数		
	  ...
	  git_check(){//获取课程特色数据
		this.$axios({method:'get',url:store.state.url_data+'/api/tags?type=1',
		   headers:{'Authorization':'Bearer '+localStorage.token}}
		   ).then(res=>{
